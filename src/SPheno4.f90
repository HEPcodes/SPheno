...
Implicit None
 ...
 Real(dp) :: BRbtosgamma, Bs_ll(3), BrBToSLL, BR_Bu_TauNu, a_e, a_mu, a_tau    &
   ...
   & , epsK, DeltaMK2, K0toPi0NuNu, KptoPipNuNu, Bd_ll(3), R_Bu_TauNu, d_N(2)
 ...
 Else If (kont.Eq.0) Then  ! models with MSSM particle content
  ...
  ! 
  !---------------------------------------------------------------------------
   If (l_SM_decoupling) then
    Call CalculateSpectrum_decoupling(n_run, delta_mass, WriteOut, kont, tanb  &
  ...
  If ((L_CS).And.(kont.Eq.0)) Then
   ...
   Do i1=1,p_max
    ...
    Call CalculateCrossSections(Ecms(i1), Pm(i1), Pp(i1), ISR(i1), Beam(i1)  &
           ...
           & , Chi0%m, N, SigC(i1,1:2,1:2), SigChi0(i1,1:4,1:4), S0%m, RS0   &
           & , vevSM, P0%m, RP0, Spm%m, RSpm, SigS0(i1,1:2), SigSP(i1,1:2,1) &
           & , SigHp(i1,1,1) )
   End Do
...
Contains
 ...
 Implicit None
  ...
  Real(dp) :: mC_T(2), mN_T(4), mS0_T(2), mSpm_T(2), mSup_T(6), mSdown_T(6)  &
    ...
    & , mass_new(32), mass_old(32), diff_m(32), sinW2, vev, Abs_mu2
  ...
  !-----------------------------------------------------
  ! MSSM  : mu and the running m_A are input
  ! MSSM1 : M_H2(i) serve as input
  ! pMSSM : mu and the running m_A are input
  !-----------------------------------------------------
  If ( (HighScaleModel(1:4).Eq."MSSM").or. & ! MSSM + MSSM1
     & (HighScaleModel.Eq."pMSSM") ) Then
   ...
   Do i1=1,n_run
    ...
    If (GenerationMixing) Then
     ...
     Call Switch_from_superCKM(Y_d, Y_u, Ad_sckm, Au_sckm, M2D_sckm &
              & , M2Q_sckm, M2U_sckm, A_d, A_u, M2_D, M2_Q, M2_U, .False. )
     If (Maxval(Abs(MatNu)).Gt.0._dp) Then
      Call Switch_from_superPMNS(Y_l, MatNu, Al_pmns, M2E_pmns, M2L_pmns &
               & , A_l, M2_E, M2_L, .False. )
     Else
    ...
    kont = 0
    If (HighScaleModel.Eq."MSSM") Then ! mu and the running m_A are input
    ...
    Else If (HighScaleModel.Eq."MSSM") Then ! m_H2(i) are input
    ...
    B_T = (M2_H_mZ(1) + M2_H_mZ(2) + 2._dp *  Abs_Mu2) * tanb_mZ &
        & / (1+tanb_mZ**2)
    ...
    If (UseNewBoundaryEW) Then
     If (GenerationMixing) then
      ...
      If (Maxval(Abs(MatNu)).Gt.0._dp) Then
       Call Switch_from_superPMNS(Y_l_mZ, MatNu, A_l_mZ, M2_E_mZ, M2_L_mZ &
...
!     & , M2_Q, M2_U, M2_H, mu, B, m_GUT, kont, .True., n_run)
 ...
 Subroutine CalculateSpectrum_decoupling(n_run, delta, WriteOut, kont, tanb &
     & , vevSM, mC, U, V, mN, N, mS0, mS02, RS0, mP0, mP02, RP0, mSpm       &
     & , mSpm2, RSpm, mSdown, mSdown2, RSdown, mSup, mSup2, RSup, mSlepton  &
     & , mSlepton2, RSlepton, mSneut, mSneut2, RSneut, mGlu, PhaseGlu       &
     & , gauge, uL_L, uL_R, uD_L, uD_R, uU_L, uU_R, Y_l, Y_d, Y_u           &
     & , Mi, A_l, A_d, A_u, M2_E, M2_L, M2_D, M2_Q, M2_U, M2_H, mu, B       &
     & , m_GUT)
 Implicit None
  Integer, Intent(in) :: n_run
  ...
  Real(dp) :: Scale_Q, g, gp, dt, tz, g1(57) &
       & , mN2(4), mC2(2), mglu_T, mP0_T(2), mP02_T(2), RP0_T(2,2)
  Real(dp) :: mass_new(32), mass_old(32), diff_m(32), sinW2, vev
  ...
  !------------------------------------------------------------------
  Iname = Iname + 1
  NameOfUnit(Iname) = "CalculateSpectrum_decoupling"
  ...
  If ( (HighScaleModel(1:4).Eq."MSSM").or. & ! MSSM + MSSM1
   ...
   mZ2_run = mZ2 ! initialisation
   mW2_run = mW2 ! initialisation
   delta_VB_MSSM = 0._dp  ! initialisation
   lamR = 0.26_dp    
   mH_SM2_tree = 126._dp**2
   Do i1=1,n_run
    !---------------------------------------
    ! the third entry will be delta_VB_BSM
    ! setting it to 0 for the moment being
    !---------------------------------------
   ...
   If (GenerationMixing) Then ! has to be improved later on
   ...
   Call delta_VB(gauge(2), 0.23_dp, sinW2_DR, 1._dp, mC, mC2, U, V, mN, mN2, N &
     & , Y_l, UL_L, UL_R, mSlepton2, Rslepton, mSneut2, RSneut, id3c           &
     & , delta_VB_MSSM, .True.)

   !------------------------------------------
   ! matching quartic coupling to m_H
   !------------------------------------------
   Call Match_lambda_SM(mS02(1), gaugeSM, Y_l_SM, Y_d_SM, Y_u_SM, delta, lamR &
                      & , kont, v_mSUSY)
   ...
   Else
    ...
    End Do
   End If
   mZ2_run = 0.25_dp * (gaugeSM(1)**2+gaugeSM(2)**2) * v_mt**2
   mW2_run = 0.25_dp * gaugeSM(2)**2 * v_mt**2
   ...
   !--------------------------
   ! I need lambda at m_Z
   !--------------------------
   tz = Log(mZ/mudim)
   dt = tz / 50._dp
   Call odeint(g4a, 59, 0._dp, tz, 0.1_dp * delta, dt, 0._dp, rge59a, kont)
                  ! TwoLoopRGE=.True.
   Call GToParameters59(g4a, gaugeSM, lamR, Y_u_SM, Y_d_SM, Y_l_SM, muSMR)
   ...
   If ((kont.Eq.0).And.(.Not.converge)) Then
    Write(ErrCan,*) 'Problem in subroutine CalculateSpectrum_decoupling!!'
    Write(ErrCan,*) "After",n_run,"iterations no convergence found"
    kont = -1200
   End If
  ...
  !--------------------------------------------------------------------------
   ...
   Call Sugra_SM_decoupling(delta, vevSM, mC, U, V, mN, N, mS0, mS02, RS0 &
     & , mP0, mP02, RP0, mSpm, mSpm2, RSpm, mSdown, mSdown2 &
     & , RSdown, mSup, mSup2, RSup, mSlepton, mSlepton2     &
     & , RSlepton, mSneut, mSneut2, RSneut, mGlu, PhaseGlu  &
     & , gauge, uL_L, uL_R, uD_L, uD_R, uU_L, uU_R, Y_l     &
     & , Y_d, Y_u, Mi, A_l, A_d, A_u, M2_E, M2_L, M2_D      &
!     & , M2_Q, M2_U, M2_H, mu, B, m_GUT, kont, .True., n_run)
     & , M2_Q, M2_U, M2_H, mu, B, lam_SM, m_GUT, kont, WriteOut, n_run)
  End If
 ...
 End Subroutine CalculateSpectrum_decoupling
